conv_data <- read.csv("conversation.csv")
...
for (k in names(final_value)){
  ...
  g_title <- paste("Tweets in ", fullname, "Conversations\n",
  ...
  plot(sort(conv_vector, decreasing = TRUE), col=symbol_color,
       pch=pitch_symbol,
       xlab="Conversation", ylab = "Number of Responses", main = g_title)
}
