...
data_train, data_test, y_train, y_test, train_filenames, test_filenames = train_test_split(
    dataset.data, dataset.target, dataset.filenames, test_size=0.2, random_state=None)
...
def size_mb(docs):
    print(docs[0][1])
    print([w.encode('utf-8') for word in docs[0].decode('utf-8') for w in word])
    return sum(len([w.encode('utf-8') for word in s.decode('utf-8') for w in word]) for s in docs) / 1e6
