...
def main(**kwarg):
    ...
    interacting_handles = [
        ...
        'britmarling', 'fredbarnes', 'MEPFuller', 'LarrySabato', 'sullydish', 'GeorgeWill', 'aglynch', 'richlowry',
        'juliaioffe', 'ahmalcolm', 'Peggynoonannyc', 'pamelageller', 'markos', 'iowahawkblog', 'majorcbs',
        'katrinanation',
        'ewerickson', 'harrisfaulkner', 'nycjim', 'mkhammer', 'AnneBayefsky', 'ShepNewsTeam', 'andylevy', 'AriMelber',
        'kirstenpowers', 'aparnapkin', 'Jonahnro', 'capehartj', 'edhenry', 'kilmeade', 'gloriasteinem', 'billhemmer',
        'AshleyJudd', 'costareports', 'mtaibbi', 'amberinzaman', 'tavissmiley', 'daveweigel', 'mattyglesias',
        'AsiaArgento',
        'marthamaccallum', 'secupp', 'katyturnbc', 'charlesmblow', 'ehasselbeck', 'monicacrowley', 'iamsambee',
        'michaelsmith', 'judgenap', 'RBReich', 'andreatantaros', 'davidcorndc', 'davidfrum', 'thereval', 'donlemon',
        'teamcavuto', 'dloesch', 'DineshDSouza', 'maureendowd', 'donnabrazile', 'mharrisperry', 'morningmika',
        'lesdoggg',
        'jemelehill', 'HardballChris', 'brithume', 'ericbolling', 'tomfriedman', 'tomilahren', 'greggutfeld',
        'krauthammer',
        'fareedzakaria', 'dianesawyer', 'ggreenwald', 'tanehisicoates', 'bretbaier', 'tuckercarlson', 'greta',
        'glennbeck',
        'lydiacachosi', 'chriscuomo', 'kimguilfoyle', 'anamariecox', 'chrislhayes', 'jaketapper', 'sarahpalinusa',
        'mitchellreports', 'anncurry', 'danaperino', 'anncoulter', 'ingrahamangle', 'JoeNBC', 'katiecouric',
        'kathygriffin',
        'nickkristof', 'michellemalkin', 'lawrence', 'jdickerson', 'megynkelly', 'camanpour', 'billoreilly',
        'NateSilver538',
        'gstephanopoulos', 'ariannahuff', 'seanhannity', 'paulkrugman', 'iamjohnoliver', 'trevornoah', 'BernieSanders',
    ...
    for counter, handle in enumerate(interacting_handles):
        if start <= counter < end:
            if handle in tweet.handles:
                ...
                """
                data = tweet.get_tweets(handle, max_tweet)
                print('Capture {:,} tweets for {}'.format(len(data), handle))
                tweet.save(handle, data)
                """
                data = []
                for vmp_tweet in tweet.get_tweets(handle, max_tweet):
                    data.append(vmp_tweet._json)
                print('Capture {:,} tweets for {}'.format(len(data), handle))
                tweet.json_save(handle, data)
